function [ct, cst, pln, resultGUI] = matRad_importDicom_custom( files, dicomMetaBool,custom_file_saver,PathName_custom,FileName_custom  )
...
if ~isempty(files.rtss)
    ...
    %% creating cst
    cst = matRad_createCst_custom(structures,FileName_custom);
...
if custom_file_saver
    mkdir (PathName_custom)
    if ischar(FileName_custom)
        switch env
        case 'MATLAB'
            clearvars -except ct cst pln stf resultGUI FileName_custom PathName_custom;
        case 'OCTAVE' 
            clear -x ct cst pln stf resultGUI FileName_custom PathName_custom;
        end
        cd(PathName_custom)
        save(FileName_custom, '-regexp', '^(?!(FileName_custom|PathName_custom)$).','-v7.3');
        %save([PathName_custom, FileName_custom], '-regexp', '^(?!(FileName_custom|PathName_custom)$).','-v7.3');
    end
else
    matRadFileName = 'test_123.mat';
